-- Run this in your Supabase SQL Editor to add the tokens system
...
-- Create trigger
DROP TRIGGER IF EXISTS update_enterprise_subscriptions_updated_at ON enterprise_subscriptions;
...
DO $$
BEGIN
    -- Enterprise subscriptions policies
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'tokens' AND policyname = 'Customers can view own tokens') THEN
        CREATE POLICY "Customers can view own tokens" ON tokens
            FOR SELECT USING (
                subscription_id IN (SELECT id FROM enterprise_subscriptions WHERE customer_id = auth.uid())
            );
    END IF;
